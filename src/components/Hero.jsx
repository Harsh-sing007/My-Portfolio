    ...
    <section ref={containerRef} id="hero" className="relative min-h-screen w-full overflow-hidden bg-[#050505]">
      ...
      <div className="hidden md:flex flex-col items-center justify-start pt-48 min-h-screen relative z-10 px-16">
        ...
        <div className="relative flex flex-col items-center z-20">
          {/* First Name - Staggered Left */}
          <motion.div 
            initial={{ x: -100, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
            className="flex overflow-hidden pb-4 lg:-ml-32"
          >
            {firstName.map((char, i) => (
              <motion.span
                key={i}
          ...
          {/* Last Name - Staggered Right */}
          <motion.div 
            initial={{ x: 100, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            transition={{ duration: 1, ease: [0.16, 1, 0.3, 1], delay: 0.2 }}
            className="flex overflow-hidden lg:-mr-32"
          >
            {lastName.map((char, i) => (
              <motion.span
                key={i}
          ...
          <div className="flex flex-col gap-6">
            {STATS.map((s, i) => (
              <div key={i} className="flex items-center gap-4">
                 <span className="text-white/10 font-display text-4xl">{i + 1}</span>
                 <div className="flex flex-col">
                   <span className="text-white text-xl font-black tracking-tighter leading-none">{s.value}</span>
                   <span className="text-white/20 text-[9px] uppercase font-bold tracking-widest">{s.label}</span>
                 </div>
              </div>
            ))}
          </div>
